...
The NeoPixels will be the most power hungry componentsa in the system. A single NeoPixel can draw up to **60 mA at full brightness**, although in this project the LEDs will typically be running at lower brightness levels a majority of the time.
...
Estimated maximum current draw: **~420 mA**
...
- The **LilyPad sewable LEDs** will be connected to digital pin **D9** which was **PWM** capability and do not need an extra power connection.
...
### NeoPixels are not lighting up

- Verify that the **power (VBATT)** and **ground (GND)** connections are properly sewn and tied off.
- Ensure the **data line from pin D12** is connected to the data input pin of the first NeoPixel. This means the arrows of the first NeoPixel should point **AWAY** from the FLORA.
- Check that the arrows on the remaining NeoPixels are all pointing down the chain and not towards the previous NeoPixel in the chain.

### Sewable LEDs are not turning on

- Confirm that the LEDs have a connection to **pin D9** and ground. Double check to ensure that none of your connections or knots came loose.
- Verify that the polarity is correct for every LED. The ground should run through the **negative (-)** pin and the signal should run through the **positive (+)** pin.
- Double check that all of your connections are secure.

### Buzzer not producing sound

- Confirm that the buzzer is connected to **pin D1 (TX)** and ground.
- Make sure the buzzer polarity is correct and not flipped.

### Tilt sensor state does not change with movement

- Ensure that the legs of the tilt sensor are **not** touching each other.
- Verify that one leg is connected to **pin D3 (SCL)** and the other to **ground**.

### Short circuits or other undefined behavior

- Double check that none of your conductive thread lines overlap. It may be helpful to flip the hoodie inside-out to check.
- Make sure you did not accidentally modify the code if that was not your intention. This troubleshooting guide may not be useful at all if you have extensively modified the code.
